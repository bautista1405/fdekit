...
Then we have the **console** package which gathers and displays all the information, traces and artifacts generated by the three packages mentioned above. [See more](https://github.com/bautista1405/fdekit/blob/main/packages/console/README.md)

The public **catalog** package contains the typed provider, connector, and recipe manifests shared by CLI scaffolding, generated documentation, and other deployment surfaces. It is metadata only and does not load provider or connector implementations.
...
The full docs index is [docs/README.md](./docs/README.md). For symbol lookup, use the API references for [`@fdekit/catalog`](./docs/api/catalog.md), [`@fdekit/core`](./docs/api/core.md), [`@fdekit/runtime`](./docs/api/runtime.md), and the [`fdekit` CLI](./docs/api/cli.md).
...
| `codebase-agent` | Search/read a customer repo, review a pull request with grounded findings and injection resistance, create an engineering issue, run codebase evals | Local sample repo, local fixture pull request, and local issue creation | GitHub, Jira, or Linear, plus Slack reviewer notifications |
