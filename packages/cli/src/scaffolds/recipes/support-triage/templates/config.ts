...
export function renderSupportTriageConfig(ctx: RecipeContext): string {
...
function importsBlock(ctx: RecipeContext): string {
  ...
  })}import { createSupportTriageMockPlanner } from '${renderRecipeModulePath(ctx, 'mock-planner.mjs')}';
...
function deploymentBlock(projectName: string): string {
  return `${renderDeploymentHeader(projectName, 'support-triage')}${renderRecipeMetadata('support-triage', [
