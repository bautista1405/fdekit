...
const publicPackages = [
  {
    packageName: '@fdekit/catalog',
    slug: 'catalog',
    packageDir: 'packages/catalog',
    stability: 'Public, pre-1.0 package-root API',
    audience: 'CLI maintainers, documentation tooling, deployment surfaces, and contributors editing built-in manifests.',
    notes: [
    ...
    topSymbols: [
      ['providerManifests', 'All built-in provider metadata in stable catalog order.'],
      ['connectorManifests', 'All built-in connector metadata in stable catalog order.'],
      ['recipeManifests', 'All built-in recipe metadata in stable catalog order.'],
      ['providerManifest', 'Resolve a provider by canonical name or supported alias.'],
      ['connectorManifest', 'Resolve a connector by canonical name or supported alias.'],
      ['recipeManifest', 'Resolve a recipe by canonical name.'],
      ['providerScaffold', 'Resolve provider scaffolding for a canonical name or alias.'],
      ['connectorScaffold', 'Resolve connector scaffolding for a canonical name or alias.'],
      ['ProviderManifest', 'Typed provider catalog entry.'],
      ['ConnectorManifest', 'Typed connector catalog entry.'],
      ['RecipeManifest', 'Typed recipe catalog entry.'],
    ],
  },
  {
    ...
    topSymbols: [
      ...
      ['EXECUTION_STATES', 'Canonical run-state vocabulary shared by CLI and hosted clients.'],
      ['ContextEnvelope', 'Versioned host-only execution context; only its model field is provider-visible.'],
      ['ModelContext', 'Explicit allowlist for provider-visible instructions, evidence, memory, skills, tools, and recent actions.'],
      ['PlannedAction', 'Immutable identity for an external effect before approval or execution.'],
      ['ApprovalRequestRecord', 'Auditable approval request bound to exact action and session state.'],
      ['InputRequestRecord', 'Typed input pause request with disclosure and resume metadata.'],
      ['ArtifactDescriptor', 'Versioned artifact identity, checksum, producer, lineage, and execution metadata.'],
      ['InferenceTarget', 'Provider/model capability identity without endpoint URLs or credentials.'],
      ['StepContextPlan', 'Budgeted per-step model context plus selected/excluded evidence manifest.'],
      ['RepositoryChangeSet', 'Immutable-base multi-file change proposal with expected blobs and permitted paths.'],
      ['ProjectSkillManifest', 'Versioned local skill provenance, capability, data, tool, mode, and eval declaration.'],
  ...
  },
  {
    ...
    topSymbols: [
      ...
      ['executeGovernedToolSequence', 'Execute exact caller-planned tools through runtime policy and resumable approvals without provider re-planning.'],
      ['resumeAgentRun', 'Resume a provider run or exact tool sequence from its recorded approved call.'],
      ...
      ['createFileArtifactDeliveryQueue', 'Commit immutable evidence versions to a restart-safe local spool.'],
      ['createHttpArtifactDeliveryTarget', 'Deliver spooled evidence with protocol and idempotency identity.'],
      ['createFileSessionStore', 'Create the durable append-only local session implementation.'],
      ['SessionStore', 'Storage-neutral append, replay, projection, and snapshot contract.'],
      ['SESSION_EVENT_TYPES', 'Standard event vocabulary for pause, retry, effects, delivery, and lifecycle evidence.'],
      ['authorizeRetrieval', 'Authorize source identities before any content is accessed.'],
      ['selectInferenceTarget', 'Match requirements to decoupled provider/model and endpoint capabilities.'],
      ['planStepContext', 'Build an allowlisted model context and exclusion manifest under budgets.'],
      ['LocalRetrievalIndex', 'Source-aware authorized exact, full-text, vector, and hybrid retrieval.'],
      ['LocalMemoryStore', 'Scoped working and episodic memory with expiry and source checks.'],
      ['LocalKnowledgeStore', 'Provenance-aware entity and relation neighborhood store.'],
      ['LocalPolicyAwareCache', 'Exact cache partitioned by tenant, policy, target, and source revisions.'],
      ['estimateInferenceUsage', 'Estimate target cost without inventing unavailable pricing.'],
      ['loadProjectSkills', 'Validate local skill manifests and entrypoint integrity without executing them.'],
      ...
      ['GovernedToolSequenceOptions', 'Input contract for deterministic governed tool sequences.'],
    ],
  },
];
